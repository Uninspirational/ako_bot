...
import com.AkoBot.Commands.BandoriGameCommands.BandoriViewCards;
...
class CommandController {
    ...
    private void commandManager(MessageReceivedEvent messageReceivedEvent, String tag) {
        ...
        switch (tag) {
            ...
            case "$gacha":
                ...
                break;
            case "$test":
                new BandoriGameCommand(messageReceivedEvent, waiter, profiles, bandoriCards);
                break;
            case "$saveall":
                profiles.saveAll();
                break;
            case "$addteam":
                retrieveProfile(messageReceivedEvent).addCardToTeam(messageReceivedEvent, waiter);
                break;
            case "$viewteam":
                retrieveProfile(messageReceivedEvent).getTeam().viewTeam(messageReceivedEvent);
                break;
            case "$viewcards":
    ...
    private Profile retrieveProfile(MessageReceivedEvent messageReceivedEvent) {
        return profiles.getProfile(messageReceivedEvent.getAuthor().getId(), bandoriCards);
    }
    private Profile loadProfile(MessageReceivedEvent messageReceivedEvent) {
        return new Profile(messageReceivedEvent.getAuthor().getId(), this.bandoriCards);
    }
}